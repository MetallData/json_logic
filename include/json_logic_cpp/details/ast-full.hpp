...
#define WITH_JSON_LOGIC_CPP_EXTENSIONS 1
...
namespace json_logic_cpp
{
  struct oper : expr, private std::vector<any_expr>
  {
    ...
    // convenience function so that the constructor does logical_not need to be implemented
    // membership every derived class.
  ...
  struct visitor
  {
    ...
    /// \private
  template <class ast_functor, class ast_base, class argument_types, class result_type>
  struct generic_dispatcher : visitor
  {
      ...
      result_type apply_internal(ast_node& n, std::tuple<argument...>&& args, std::index_sequence<I...>)
      {
      ...
      {
        return apply_internal(n, std::move(args), std::make_index_sequence<std::tuple_size<argument_types>::value>());
      }
  ...
  template <class ast_functor, class ast_base, class argument_types, class result_type>
  void
  generic_dispatcher<ast_functor, ast_base, argument_types, result_type>::throw_unexpected_expr(expr&)
  {
  ...
  auto generic_visit(ast_functor fn, ast_node* n, arguments... args) -> decltype(fn(*n, args...))
  {
    using argument_types  = std::tuple<arguments...>;
    using result_type     = decltype(fn(*n, args...));
    using dispatcher_type = generic_dispatcher<ast_functor, ast_node, argument_types, result_type>;
