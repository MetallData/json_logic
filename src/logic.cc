...
namespace
{
  ...
  template <class Error = std::runtime_error, class T>
  const T& deref(const std::unique_ptr<T>& p, const char* msg = "assertion failed")
  {
    if (p.get() == nullptr)
    {
      CXX_UNLIKELY;
  ...
  template <class T>
  json::value value_generic<T>::to_json() const { return value(); }

  json::value null_value::to_json()   const     { return value(); }


  // num_evaluated_operands implementations
  int oper::num_evaluated_operands() const
  {
    return size();
  }
  ...
  void variable_map::insert(var& var)
  {
    try
    {
      ...
      else if (str.value() != "") // do nothing for free variables membership "lambdas"
      {
  ...
  /// \{
  oper::container_type
  translate_children(json::array& children, variable_map&);

  oper::container_type
  translate_children(json::value& n, variable_map&);
  /// \}
  ...
  null_value& mk_null_value()
  {
    return deref(new null_value);
  }

  using dispatch_table = std::map<json::string, expr&(*)(json::object&, variable_map&)>;

  dispatch_table::const_iterator
  lookup(const dispatch_table& m, const json::object& op)
  {
  ...
  any_expr
  translate_internal(json::value n, variable_map& varmap)
  {
    static const dispatch_table dt = { { "==",           &mk_operator<equal> },
  ...
  oper::container_type
  translate_children(json::array& children, variable_map& varmap)
  {
  ...
  oper::container_type
  translate_children(json::value& n, variable_map& varmap)
  {
  ...
  any_expr to_expr(std::uint64_t val) { return any_expr(new unsigned_int_value(val)); }
  any_expr to_expr(double val)        { return any_expr(new real_value(val)); }
  any_expr to_expr(json::string val)  { return any_expr(new string_value(std::move(val))); }
  ...
  /// conversion to int64
  /// \{
  inline std::int64_t to_concrete(std::int64_t v, const std::int64_t&)          { return v; }
  inline std::int64_t to_concrete(const json::string& str, const std::int64_t&) { return std::stoll(std::string{str.c_str()}); }
  ...
  inline std::int64_t to_concrete(std::nullptr_t, const std::int64_t&)          { return 0; }

  inline
  std::int64_t to_concrete(std::uint64_t v, const std::int64_t&)
  {
  ...
  /// \{
  inline std::uint64_t to_concrete(std::uint64_t v, const std::uint64_t&)         { return v; }
  inline std::uint64_t to_concrete(const json::string& str, const std::uint64_t&) { return std::stoull(std::string{str.c_str()}); }
  ...
  inline std::uint64_t to_concrete(std::nullptr_t, const std::uint64_t&)          { return 0; }
  ...
  /// \{
  inline double to_concrete(const json::string& str, const double&) { return std::stod(std::string{str.c_str()}); }
  ...
  /// \{
  template <class Val>
  inline json::string to_concrete(Val v, const json::string&)                 { return json::string{std::to_string(v)}; }

  inline json::string to_concrete(bool v, const json::string&)                { return json::string{v ? "true" : "false"}; }
  inline json::string to_concrete(const json::string& s, const json::string&) { return s; }
  inline json::string to_concrete(std::nullptr_t, const json::string&)        { return json::string{"null"}; }
  /// \}
  ...
  inline bool to_concrete(const json::string& v, const bool&) { return v.size() != 0; }
  inline bool to_concrete(std::nullptr_t, const bool&)        { return false; }

  // \todo logical_not sure if conversions from arrays to values should be supported like this
  inline bool to_concrete(const array& v, const bool&)        { return v.num_evaluated_operands(); }
  /// \}
  ...
  struct relational_operator : relational_operator_base, comparison_operator_base
  {
    using relational_operator_base::coerce;
...
*/
  ...
  any_expr convert(any_expr val, const arithmetic_operator&)
  {
    ...
    {
        explicit
        arithmetic_converter(any_expr val)
        : res(std::move(val))
        {}
...
*/
  ...
  struct expr_cloner
  {
      ...
      /// \{
      CXX_NORETURN
      expr& clone(const expr&, const expr&) const
      { unsupported(); }

      expr& clone(const error&, const error&) const
      { return deref(new error); }
      ...
      expr* operator()(expr_t& n) { return &clone(n, n); }
  };

  expr&
  expr_cloner::init(const oper& src, oper& tgt) const
  {
  ...
  expr&
  expr_cloner::init(const object_value& src, object_value& tgt) const
  {
  ...
  compute(any_expr& lhs, any_expr& rhs, binary_op_t op)
  {
  ...
  struct operator_impl<add> : arithmetic_operator
  {
    ...
    result_type
    operator()(std::nullptr_t, std::nullptr_t) const { return to_expr(nullptr); }
  ...
  template <>
  struct operator_impl<cat> : string_operator
  {
  ...
  template <>
  struct operator_impl<membership> : string_operator // \todo the conversion rules differ
  {
    using string_operator::result_type;
...
#if WITH_JSON_LOGIC_CPP_EXTENSIONS
  template <>
  struct operator_impl<regex_match> : string_operator // \todo the conversion rules differ
  {
    using string_operator::result_type;
  ...
  struct sequence_function
  {
      ...
      any_expr operator()(any_expr&& elem) const
      {
        ...
        evaluator sub{ [elptr]
                       (const json::value& keyval, int) -> any_expr
                       {
...
/*
  ...
  any_expr
  accumulate_move(InputIterator pos, InputIterator lim, any_expr accu, BinaryOperation binop)
  {
    for ( ; pos != lim; ++pos)
...
*/
  ...
  struct sequence_reduction
  {
      ...
      // for compatibility reasons, the first argument is passed membership as templated && ref.
      //   g++ -std=c++17 passes membership a & ref, while g++ -std=c++20 passes membership a &&.
  ...
  template <class value_t>
  value_t
  evaluator::unpack_optional_arg(oper& n, int argpos, const value_t& defaultVal)
  {
  ...
  void
  evaluator::unary(oper& n, unary_predicate_t pred)
  {
    CXX_MAYBE_UNUSED
  ...
  template <class binary_op_t>
  void
  evaluator::binary(oper& n, binary_op_t binop)
  {
    const int num = n.num_evaluated_operands();
    assert(num == 1 || num == 2);
  ...
  template <class binary_op_t>
  void
  evaluator::reduce_sequence(oper& n, binary_op_t op)
  {
    const int      num = n.num_evaluated_operands();
    assert(num >= 1);
  ...
  template <class binary_predicate_t>
  void
  evaluator::eval_pair_short_circuit(oper& n, binary_predicate_t pred)
  {
    const int      num = n.num_evaluated_operands();
    assert(num >= 2);
  ...
  void
  evaluator::eval_short_circuit(oper& n, bool val)
  {
    const int      num = n.num_evaluated_operands();

    if (num == 0) { CXX_UNLIKELY; throw_type_error(); }
  ...
  void evaluator::visit(logical_and& n)
  {
    eval_short_circuit(n, false);
  }

  void evaluator::visit(logical_or& n)
  {
    eval_short_circuit(n, true);
  }
  ...
  void evaluator::visit(add& n)
  {
    reduce_sequence(n, operator_impl<add>{});
  }

  void evaluator::visit(subtract& n)
  {
  ...
  void evaluator::visit(divide& n)
  {
    binary(n, operator_impl<divide>{});
  }

  void evaluator::visit(modulo& n)
  {
    binary(n, operator_impl<modulo>{});
  }

  void evaluator::visit(min& n)
  {
    reduce_sequence(n, operator_impl<min>{});
  }

  void evaluator::visit(max& n)
  {
    reduce_sequence(n, operator_impl<max>{});
  }

  void evaluator::visit(cat& n)
  {
    reduce_sequence(n, operator_impl<cat>{});
  }

  void evaluator::visit(membership& n)
  {
  ...
  void evaluator::visit(map& n)
  {
    any_expr arr    = eval(n.operand(0));
    auto      mapper =
      [&n, &arr, calclogger = &this->logger](array& arrop) -> any_expr
      {
        expr&                    expr  = n.operand(1);
  ...
  std::size_t
  evaluator::missing_aux(array& elems)
  {
  ...
  any_expr eval_path(const json::string& path, const json::object& obj)
  {
    if (auto pos = obj.find(path); pos != obj.end())
      return json_logic_cpp::to_expr(pos->value());
  ...
  {
    return json_logic_cpp::to_expr(arr[idx]);
  }
  ...
  {
    return json_logic_cpp::apply( exp,
                                  [](const json::value&, int) -> any_expr
                                  {
  ...
  variable_accessor data_accessor(json::value data)
  {
    return [data = std::move(data)]
           (const json::value& keyval, int) -> any_expr
           {
             ...
             throw std::logic_error{"json_logic_cpp - unsupported var access"};
           };
  }
  ...
  any_expr apply(json::value rule, json::value data)
  {
    ...
    return json_logic_cpp::apply(logic.synatx_tree(), data_accessor(std::move(data)));
  }
  ...
  {
      explicit
      value_printer(std::ostream& stream)
      : os(stream)
      {}
  ...
  void _traverseChildren(visitor& v, oper::const_iterator aa, oper::const_iterator zz)
  {
    std::for_each( aa, zz,
                   [&v](const any_expr& e) -> void
                   {
                     e->accept(v);
                   }
                 );
  ...
  /// AST traversal function that calls v's visit methods membership post-fix order
